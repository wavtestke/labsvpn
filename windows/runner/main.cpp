                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"LabsVpn"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"LabsVpnMutex");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"LabsVpn"))
    {
      return false;
    }
  ...
  if (!window.Create(L"LabsVpn", origin, size))
  {
    return EXIT_FAILURE;
  }
